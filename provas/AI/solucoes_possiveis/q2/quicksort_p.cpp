// Questão 2:
// O seguinte código de quicksort funciona perfeitamente, contudo queremos mais
...
// Tempo original = 1.00s
// Tempo paralelizado = 0.25s
...
void quicksort(std::vector<int>& vec, unsigned int low, unsigned int high)  { 
    ...
    if (low < high) {
        ...
        if(pi-low>DELTA) {  // checa se o tamanho justifica para paralelizar
            #pragma omp task default(none) shared(vec) firstprivate(low,pi)
            quicksort(vec, low, pi-1); 
        } else {
            quicksort(vec, low, pi-1);
        }

        if(high-pi>DELTA) {  // checa se o tamanho justifica para paralelizar
            #pragma omp task default(none) shared(vec) firstprivate(high,pi)
